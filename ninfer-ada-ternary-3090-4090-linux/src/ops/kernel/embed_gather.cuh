...
// Prism 三元词表（组宽 128）。一个线程解出一个输出元素：先由 token id 定位源行，再定位该元素
// 所属的 128 权重组，然后把该组的三个平面指针交给与三元线性层【同一个】解码原子 —— 这样词表与
// 线性权重不可能各解各的。PQ2_0 传 high == nullptr（row_split_geometry 为它报的高位平面字节数为
// 零）；PTQ1_0 从高位平面读 qh。
