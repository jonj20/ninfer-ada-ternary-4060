...
// Prism 三元词表：组宽 128，基平面（qs）+ 可选高位平面（qh，仅 PTQ1_0）+ 每组一个 binary16 缩放。
// 平面几何与 row_split_geometry() 一致，其尺寸已逐字节对照制品读取器核验过
...
// Prism 三元词表把行折叠存进旋转基，所以查出来的一行还不是可入残差流的激活：模型在查表之后
// 计算 h = s * (H * z)，这是整个模型里唯一一处逆向映射。
//
// 就地映射不花任何 scratch，这正是"无工作区的 embedding 算子"签名得以保持不变的原因。
// NINFER_TERNARY_HADAMARD=0 时这个映射与其它折叠基变换一起被跳过。
...
// 诊断钩子：NINFER_TERNARY_DUMP_EMBED=<file> 会把 token id 与【最终】的 embedding 激活（逆向
// 映射之后）写盘，供外部 oracle 拿制品的载荷核对整条查表路径。残差流正是从这里进入第 0 层，
// 所以这一点断掉会让后面所有数字失去意义 —— 表现为困惑度接近均匀分布。
// 落盘的是原始 BF16，由 oracle 自行加宽。输出路径必须是纯 ASCII。
void dump_embedding_if_requested(const Tensor& ids, const Tensor& out, cudaStream_t stream,
    ...
    // 构图会把本算子重放一遍，而那时流正在捕获：下面的 device->host 拷贝与"让 dump 有意义的"
    // 同步在捕获期都是非法的。跳过捕获意味着 dump 落在第一次真实重放上，而那一次才是真正在意
    // 的数值。
    ...
    // 每个 (阶段, token 数) 一个文件，这样"查出来的行"和"映射后的行"能区分开，且 prefill 的
    // dump 不会被之后的 decode dump 覆盖。
    char suffixed[1024];
