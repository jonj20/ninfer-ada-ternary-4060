...
def evaluate(tag: str, k: int, tokens: int, hd_n: int, nk_n: int, rep_n: int, inverse: bool,
    ...
    # ninfer/ggml 约定 ne[0] 连续，所以 [k, tokens] 激活是 token 主序：
    # 元素 (column, token) 落在 token * k + column。用 C 序 reshape 会把它读成行主序，
    # 于是 T == 1 全绿、T > 1 全错 —— 这正是旧版 oracle 的读数方式。
    x = x.reshape(k, tokens, order="F")
    got = got.reshape(k, tokens, order="F")
