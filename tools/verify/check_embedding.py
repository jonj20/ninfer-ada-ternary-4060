...
def main() -> int:
    ...
    # 引擎侧 embed_gather_* 内核一律按 out[token * d + k] 写：缓冲是【token 主序】，尽管张量
    # 的 extents 读作 [hidden, T]。所以要先按 F 序还原成 (hidden, tokens)，元素 (k, t) 才落在
    # t*hidden + k；用 C 序会被静默转置，T > 1 时全部对不上，看起来像引擎整体坏了，而 T == 1
    # 时两种读法完全重合、什么都发现不了。
    flat = np.frombuffer(raw[12 + id_count * 4:], dtype=np.uint16)
    got = (flat.astype(np.uint32) << 16).view(np.float32).astype(np.float64).reshape(
        hidden, tokens, order="F")
